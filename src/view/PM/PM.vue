      ...
      <div class="PMSection">
        ...
        <div class="select-fax">
          <div class="item-select">
            ...
            <el-select @change="SelectList" v-model="SelectStatus" placeholder="请选择">
              <el-option
          ...
          <div class="item-select">
            <div class="SearchBox">
              <input type="text" placeholder="请输入关键字进行模糊搜索"  @keyup.enter="SelectList" v-model="SearchPM">
              <div class="SearchBut" @click="SelectList"></div>
            </div>
          </div>
        </div>
        ...
        <div class="ProList">
          <div class="ProN">
            <div class="ProHeader">
              <div class="ProNum">项目编号</div>
              <div class="ProName">项目名称</div>
              <div class="ProName">状态</div>
              <div class="ProCreatName">创建人</div>
              <div class="CrateTime">创建日期</div>
              <div class="Operation">操作</div>
            </div>
            <div class="proMenu">
              <div class="procant" v-for="(item,index) in ProList" :key="index">
                <div class="ProNum">{{item.pro_number}}</div>
                <div class="ProName">{{item.name}}</div>
                <div class="ProName">{{item.status}}</div>
                <div class="ProCreatName">{{item.user}}</div>
                <div class="CrateTime">{{item.create_time}}</div>
                <div class="Operation">
                <div>
                ...
                <div>
                  <img src="../../assets/img/accomplish.png" alt="" @click="GetOnecPro(item.id,item.pro_number)">
                  <span>完成</span>
                </div>
                <div>
      ...
      methods:{
        ...
        getOnecPro(id){
          this.$http.get(this.$conf.env.GetOnecPro + `${id}/`).then(res=>{
            ...
            this.value = res.data.message.page_system.id
            this.ProID = res.data.message.id
        ...
        //获取某一页
        getProList(){
          this.$http.get(this.$conf.env.GetProList + `?page=${this.CurrentChange}&search=${this.SearchPM}&status=${this.SelectStatus}&page_size=${this.PageSize}`).then(res=>{
        ...
        getSystem(){
          this.$http.get(this.$conf.env.GetSystem).then(res=>{
            ...
            for(var i in res.data.message){
              let a = {
                value:res.data.message[i].id,
                label:res.data.message[i].name
              }
              this.options.push(a)
            }
          }).catch(err=>{
        ...
        RemovePro(id,name){
          this.$confirm(`此操作将永久删除${name}, 是否继续?`, '提示', {
            ...
            this.$http.delete(this.$conf.env.GetOnecPro + `${id}/`).then(res=>{
        ...
        .ProList{
          ...
          .ProN{
            ...
            .ProHeader{
              ...
              .ProNum,.Operation,.ProName,.CrateTime{
                width: 15%;
                height: .5rem;
              }
            }
