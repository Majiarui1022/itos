  ...
  <div class="PMContent">
      ...
      <div class="PerSonneSection">
        ...
        <div class="ProList">
          ...
          >
            ...
            <el-table-column prop="name" :align="'center'" label="权限组名"></el-table-column>
              ...
              <template slot-scope="scope">
                <div>
                  <img
                    ...
                    alt
                    @click="showZuDia(2,scope.row.id,scope.row.name)"
                  />
                  <span>编辑</span>
                </div>
                <div>
                  <img src="../../assets/img/qx.png" alt @click="pqx(scope)" />
                  <span>权限</span>
                </div>
                <div>
                  <img
    ...
    <el-dialog 
      ...
      <el-form ref="form" :model="form">
        ...
        >
          <el-input v-model="form.name" placeholder="请输入15位中文英文组合" autocomplete="off" maxlength="15"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    //获取权限组列表
    getqxlist(){
      this.$http.get(this.$conf.env.createqx + `?page=${this.CurrentChange}&page_size=${this.PageSize}`).then(res=>{
        console.log(res)
    ...
    createZuname(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if(this.titleName == '创建组名'){
             this.$http.post(this.$conf.env.createqx,this.form).then(res=>{
              console.log(res)
            ...
            this.$http.put(this.$conf.env.createqx + `${this.zuID}/`,this.form).then(res=>{
              console.log(res)
              if(res.code == -1)return
       ...
       //删除某权限
        RemovePero(id,name){
          this.$confirm(`此操作将永久删除权限组 ${name}, 是否继续?`, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
            }).then(() => {
            this.$http.delete(this.$conf.env.createqx + `${id}/`).then(res=>{
              this.$message({
                type: 'success',
                duration:1000,
                message: '删除成功!'
